...
--FROM Items i
--JOIN ItemAmenities ia ON ia.ItemID = i.ID
--JOIN Amenities a ON ia.AmenityID = a.ID;
...
-- USER CORE  (thông tin chung + xác thực)
-- IsAdmin: thay thế UserTypes cho role quản trị
-- Để biết user là host/guest: kiểm tra sự tồn tại trong Hosts/Guests
...
-- GUEST PROFILE  (1-1 optional với Users)

-- Chỉ tồn tại khi user đã từng đặt phòng hoặc tự đăng ký role guest
-- LoyaltyPoints: cộng khi Booking → Completed, trừ khi đổi điểm
...
-- HOST PROFILE  (1-1 optional với Users)

-- Chỉ tồn tại khi user đăng ký host và được duyệt
-- Rating: cache avg từ Reviews, cập nhật bằng trigger/app
...
-- SECTION 9 — HOST BANK ACCOUNTS
-- AccountNumber được mã hóa ở application layer trước khi lưu
...
CREATE TABLE [dbo].[Items](
    [ID]                 [bigint]           IDENTITY(1,1) NOT NULL,
    [GUID]               [uniqueidentifier] NOT NULL DEFAULT NEWID(),
    [HostUserID]         [bigint]           NOT NULL,   -- FK → Hosts (thay vì Users)
...
CREATE TABLE [dbo].[Bookings](
    [ID]                   [bigint]           IDENTITY(1,1) NOT NULL,
    [GUID]                 [uniqueidentifier] NOT NULL DEFAULT NEWID(),
    [GuestUserID]          [bigint]           NOT NULL,   -- FK → Guests (rõ hơn UserID)
    [ItemID]               [bigint]           NOT NULL,
...
-- SECTION 14 — FOREIGN KEY CONSTRAINTS
...
-- HostBankAccounts → Hosts
ALTER TABLE [dbo].[HostBankAccounts]
    ADD CONSTRAINT [FK_HostBankAccounts_Hosts]
    FOREIGN KEY ([HostUserID]) REFERENCES [dbo].[Hosts]([UserID])
GO

-- Items → Hosts (bắt buộc phải là host mới đăng ký được chỗ ở)
ALTER TABLE [dbo].[Items]
    ADD CONSTRAINT [FK_Items_Hosts]
    FOREIGN KEY ([HostUserID]) REFERENCES [dbo].[Hosts]([UserID])
GO
ALTER TABLE [dbo].[Items]
...
-- Bookings → Guests (không phải Users → tránh đặt phòng khi chưa có profile guest)
ALTER TABLE [dbo].[Bookings]
    ADD CONSTRAINT [FK_Bookings_Guests]
    FOREIGN KEY ([GuestUserID]) REFERENCES [dbo].[Guests]([UserID])
GO
ALTER TABLE [dbo].[Bookings]
...
GO
CREATE NONCLUSTERED INDEX [IX_Users_IsActive] ON [dbo].[Users]([IsActive]) INCLUDE ([Username], [IsAdmin])
GO

-- Hosts / Guests
CREATE NONCLUSTERED INDEX [IX_Hosts_IsVerified]  ON [dbo].[Hosts]([IsVerified]) INCLUDE ([Rating])
GO
CREATE NONCLUSTERED INDEX [IX_Guests_Loyalty]    ON [dbo].[Guests]([LoyaltyPoints])
GO
CREATE NONCLUSTERED INDEX [IX_HostBankAccounts_HostUserID]
    ON [dbo].[HostBankAccounts]([HostUserID]) INCLUDE ([IsPrimary],[IsVerified])
GO

-- Items
CREATE NONCLUSTERED INDEX [IX_Items_HostUserID]       ON [dbo].[Items]([HostUserID])
GO
CREATE NONCLUSTERED INDEX [IX_Items_AreaID]            ON [dbo].[Items]([AreaID])
GO
CREATE NONCLUSTERED INDEX [IX_Items_IsActive_AreaID]   ON [dbo].[Items]([IsActive],[AreaID])
    INCLUDE ([HostUserID],[ItemTypeID],[Capacity])
GO

-- ItemPrices / ItemAvailability
...
-- Bookings
CREATE NONCLUSTERED INDEX [IX_Bookings_GuestUserID]   ON [dbo].[Bookings]([GuestUserID])
GO
CREATE NONCLUSTERED INDEX [IX_Bookings_ItemID]         ON [dbo].[Bookings]([ItemID])
GO
CREATE NONCLUSTERED INDEX [IX_Bookings_CheckInDate]
    ON [dbo].[Bookings]([CheckInDate]) INCLUDE ([CheckOutDate],[ItemID],[GuestUserID])
GO
CREATE NONCLUSTERED INDEX [IX_Bookings_Status]
    ON [dbo].[Bookings]([BookingStatus]) INCLUDE ([GuestUserID],[ItemID],[FinalPrice])
GO
...
-- SECTION 16 — VIEWS tiện ích

-- View kiểm tra nhanh role của từng user
...
-- View thay thế TotalBookings (không lưu, tính động)
...
-- ---- Users ----------------------------------------------
-- Gồm: admin, host-only, guest-only, và user vừa host vừa guest
-- ID 1        = admin
-- ID 2,3,4,6  = host (có thể đặt phòng → cũng là guest)
-- ID 7–13     = guest-only
...
-- Hosts (cũng là guest → sẽ có hàng trong cả Hosts lẫn Guests)
...
-- ---- Guests (IDs: 2,3,4,6 = host CŨNG là guest; 7-13 = guest-only)
...
-- ---- Items (HostUserID thay vì UserID) ------------------
...
-- Host Mahdi (ID=3) đặt phòng của người khác → cũng là guest
(6, 3, 1, 190.00, '2022-10-23', N'GW-2022-006')
SET IDENTITY_INSERT [dbo].[Transactions] OFF
GO

-- ---- Bookings (mẫu) — dùng GuestUserID ------------------
...
-- Host Mahdi (ID=3) đặt phòng của host khác → minh chứng Option B
...
-- Mahdi (host) đánh giá chỗ ở sau khi đặt phòng → Option B trong thực tế
(6, 6,  3, NULL, 23, 5, N'Lovely quiet place. Will recommend to guests!')
SET IDENTITY_INSERT [dbo].[Reviews] OFF
GO

PRINT '====================================================='
PRINT ' Seoul_Stay v3 created successfully.'
PRINT ' Tables  : 22 (+ Guests, Hosts, HostBankAccounts)'
PRINT ' Views   : 3  (vw_UserRoles, vw_GuestBookingSummary,'
PRINT '               vw_HostPerformance)'
PRINT ' Indexes : 20'
PRINT ' Pattern : TPT Option B — user có thể vừa Host vừa Guest'
PRINT '====================================================='
GO